...
public class UpdateRevisionImpl {
    ...
    public RevisionResponse execute(Integer id, CreateRevisionRequest req) {
        ...
        if (req.getIdContact() == null ||
                req.getIdType() == null ||
                req.getObservations() == null || req.getObservations().trim().isEmpty() ||
                req.getRevisionDate() == null) {
            throw new RevisionInvalidDataException("Faltan datos obligatorios en la petición de actualización");
        }

        // 3. Buscar Contact
        Contact contact = contactRepository.findById(req.getIdContact())
                .orElseThrow(() -> new ContactNotFoundException(req.getIdContact()));

        // 4. Buscar RevisionType
        RevisionType type = revisionTypeRepository.findById(req.getIdType())
        ...
        if (req.getIdProtocol() != null) {
            protocol = protocolRepository.findById(req.getIdProtocol())
        ...
        if (idPac != null) {
            patientRepository.findById(idPac)
                    .orElseThrow(() -> new PatientNotFoundException(idPac));
        }
