...
class GetVisibleRevisionsImplTest {
  ...
  @Test
  void execute_returnsMappedVisibleRevisions() {
    ...
    when(revisionRepository.findAllByIsVisibleTrue()).thenReturn(Arrays.asList(rev1, rev2));
    when(revisionMapper.toDto(rev1)).thenReturn(resp1);
    when(revisionMapper.toDto(rev2)).thenReturn(resp2);

    List<RevisionResponse> result = service.execute();
    ...
    verify(revisionRepository).findAllByIsVisibleTrue();
