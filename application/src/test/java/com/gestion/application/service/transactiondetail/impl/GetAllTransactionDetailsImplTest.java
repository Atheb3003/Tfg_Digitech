...
class GetAllTransactionDetailsImplTest {

  @Mock private TransactionDetailRepository detailRepo;

  @Mock private TransactionDetailMapper mapper;

  @InjectMocks private GetAllTransactionDetailsImpl service;
  ...
  void getAll_shouldReturnMappedList() {
    ...
    when(detailRepo.findAll()).thenReturn(List.of(detail1, detail2));
    when(mapper.toResponse(detail1)).thenReturn(resp1);
    when(mapper.toResponse(detail2)).thenReturn(resp2);

    List<TransactionDetailResponse> result = service.getAll();
