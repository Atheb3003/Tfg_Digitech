...
class GetContactsWithPatientImplTest {

  @Mock private ContactRepository contactRepository;

  @Mock private ContactMapper contactMapper;

  @InjectMocks private GetContactsWithPatientImpl getContactsWithPatient;
  ...
  @Test
  void shouldReturnPageOfContactsWithPatient() {
    // Given
    ...
    when(contactRepository.findAllWithPatientOrderByIdDesc(pageable)).thenReturn(contactPage);
    when(contactMapper.toResponse(contact1)).thenReturn(response1);
    when(contactMapper.toResponse(contact2)).thenReturn(response2);

    // When
    Page<ContactResponse> result = getContactsWithPatient.getContactsWithPatient(pageable);
    ...
    verify(contactRepository).findAllWithPatientOrderByIdDesc(pageable);
