...
public class GetTotalIncomeImpl {
    ...
    public TotalIncomeResponse getTotalIncome(LocalDate startDate, LocalDate endDate) {
        ...
        LocalDate minDate = all.stream()
                .map(td -> td.getTransaction().getTransactionDate())
                .min(LocalDate::compareTo).orElse(LocalDate.now());
        LocalDate maxDate = all.stream()
                .map(td -> td.getTransaction().getTransactionDate())
        ...
        double total = all.stream()
                ...
                .mapToDouble(td -> td.getPrice() != null ? td.getPrice().doubleValue() * (td.getQuantity() == null ? 1 : td.getQuantity()) : 0.0)
                .sum();
