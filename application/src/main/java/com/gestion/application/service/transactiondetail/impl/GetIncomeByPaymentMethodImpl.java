...
public class GetIncomeByPaymentMethodImpl {
  ...
  public IncomeByPaymentMethodResponse getIncomeByPaymentMethod(
    ...
    if (all.isEmpty()) {
      return new IncomeByPaymentMethodResponse(
          Collections.emptyMap(), formatAmount(0.0), startDate, endDate);
    }
    ...
    List<TransactionDetail> filtered =
        all.stream()
            .filter(
                td -> {
    ...
    return new IncomeByPaymentMethodResponse(
        formatted, formatAmount(total), effectiveStart, effectiveEnd);
  }
}
