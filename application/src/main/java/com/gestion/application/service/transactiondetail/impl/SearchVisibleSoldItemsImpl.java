...
public class SearchVisibleSoldItemsImpl {
    ...
    public Page<SoldItemResponse> searchVisibleSoldItems(String term, Pageable pageable) {
        ...
        for (TransactionDetail td : allVisible) {
            ...
            for (int i = 0; i < quantity; i++) {
                ...
                // --- Mismo mapeo que tienes actualmente ---
                ...
                if ("PRODUCT".equals(dto.getType())) {
                    ...
                    dto.setName(
                            td.getSurgeryReservation().getSurgicalTechnique() + " " +
                                    td.getSurgeryReservation().getFollicularUnits() + "UF"
                    );
                }
                ...
                if (td.getTransaction() != null &&
                    ...
                    dto.setIdContactString(td.getTransaction().getPatient().getContact().getIdContactString());
                }
                ...
                boolean matches =
                        (dto.getName() != null && dto.getName().toLowerCase().contains(search)) ||
                                (dto.getAmount() != null && dto.getAmount().toString().contains(search)) ||
                                (dto.getPaymentMethod() != null && dto.getPaymentMethod().toString().toLowerCase().contains(search)) ||
                                (dto.getIdContactString() != null && dto.getIdContactString().toLowerCase().contains(search));
        ...
        List<SoldItemResponse> pageContent = start > end ? new ArrayList<>() : result.subList(start, end);
