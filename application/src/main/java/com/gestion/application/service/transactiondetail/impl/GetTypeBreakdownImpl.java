    ...
    public TypeBreakdownResponse getTypeBreakdown(LocalDate startDate, LocalDate endDate) {
        ...
        if (startDate == null || endDate == null) {
            ...
            if (visibles.isEmpty()) {
                ...
                if (startDate == null)
                    startDate = visibles.stream().map(td -> td.getTransaction().getTransactionDate()).min(LocalDate::compareTo).orElse(LocalDate.now());
                if (endDate == null)
                    endDate = visibles.stream().map(td -> td.getTransaction().getTransactionDate()).max(LocalDate::compareTo).orElse(LocalDate.now());
            }
        }

        List<TransactionDetail> details = transactionDetailRepository.findAllByIsVisibleAndTransactionDateBetween(startDate, endDate);
