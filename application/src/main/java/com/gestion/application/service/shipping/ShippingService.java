...
public class ShippingService {
  ...
  public Page<ShippingResponse> obtenerTodos(Pageable pageable) {
    return shippingRepository.findAllByOrderByIdDesc(pageable).map(ShippingMapper::toResponse);
  }

  public Optional<ShippingResponse> obtenerPorId(Long id) {
    return shippingRepository.findById(id).map(ShippingMapper::toResponse);
  }
